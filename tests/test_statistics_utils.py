...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_moving_average_only_accepts_1d_sequences(self):
        ...
        with self.assertRaises(ValueError):
            utils.moving_average(array2d, window=2)
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        self.assertAlmostEqual(z.mean(), 0.0, places=7)

        self.assertAlmostEqual(z.std(), 1.0, places=7)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        scaled = utils.min_max_scale(arr)

        self.assertAlmostEqual(scaled.min(), 0.0, places=7)

        self.assertAlmostEqual(scaled.max(), 1.0, places=7)

        expected = np.array([0.0, 0.5, 1.0])
        npt.assert_allclose(scaled, expected, rtol=1e-10, atol=1e-10)
